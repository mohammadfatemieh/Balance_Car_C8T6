...
{			  
	...
	for(uint8_t i=0; i<8; i++)
	{			  
		OLED_SCL_LOW();			//OLED_SCLK=0;
		if(dat&0x80)
			OLED_SDA_HIGH();	//OLED_SDIN=1;
		else
			OLED_SDA_LOW();		//OLED_SDIN=0;
		OLED_SCL_HIGH();		//OLED_SCLK=1;
		dat<<=1;   
	}				 
												//OLED_CS=1;略	  
	OLED_DC_HIGH();				//OLED_RS=1;   	  
} 
...
{ 						  
												//OLED_CS=1;略
	OLED_DC_HIGH();				//OLED_RS=1;	 
	OLED_RST_LOW();				//OLED_RST=0;

	for(uint16_t i = 7200; i > 0; i--)	//代替延时100ms
		for(uint16_t j = 100; j > 0; j--);
	
	OLED_RST_HIGH();			//OLED_RST=1; 
